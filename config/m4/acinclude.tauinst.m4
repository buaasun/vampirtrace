...
[
	...
	AC_ARG_WITH(pdt-fparse,
		...
		[give the command for PDT Fortran source code parser (gfparse,f95parse,f90parse), default: first found in the list]),
	[
		AS_IF([test x"$withval" = "xyes" -o x"$withval" = "xno"],
	...
	[
		...
		[
			...
			AS_IF([test x"$FC" != x],
			[
				AC_CHECK_PROGS(tauinst_fparse_cmd, gfparse f95parse f90parse)
				AS_IF([test x"$tauinst_fparse_cmd" = x],
				[AC_MSG_WARN([no gfparse, f95parse, or f90parse found; Fortran source code cannot be instrumented by TAU])])
			],
			[
				tauinst_fparse_cmd=
			])
