                            ...
                            MethodInfoT* methodInfo)
{
  ...
  {
    error = (*jvmti)->GetSourceFileName(jvmti, class, &source_filename);
    if ( error != JVMTI_ERROR_ABSENT_INFORMATION )
      VT_JAVA_CHECK_ERROR(jvmti, error, "GetSourceFileName");

    if ( error == JVMTI_ERROR_NONE )
    {
      error = (*jvmti)->GetLineNumberTable(jvmti, method, &line_count, &lines);
      if ( error != JVMTI_ERROR_ABSENT_INFORMATION )
        VT_JAVA_CHECK_ERROR(jvmti, error, "GetLineNumberTable");

      if ( error == JVMTI_ERROR_NONE )
      {
        vt_libassert(source_filename != NULL);
        vt_libassert(lines != NULL && line_count > 0);
